...
var AdguardSelectorLib = (function ($) { // jshint ignore:line
   ...
   // PRIVATE FIELDS
    ...
    var BorderSelectionRenderer = (function (api) {
        var BORDER_WIDTH = 3;//选择边框  宽度
        var BORDER_PADDING = 2;//不影响选择，只影响 .add
        ...
        api.init = function () {
            if (!borderTop) {
                ...
                borderTop = $('<div/>').addClass(BORDER_CLASS).css('height', width)
                    .on("click", sgMousedownHandler);
                borderBottom = $('<div/>').addClass(BORDER_CLASS).addClass('sg_bottom_border')
                    .css('height', px(BORDER_WIDTH + 9))
                    .on("click", sgMousedownHandler);
                borderLeft = $('<div/>').addClass(BORDER_CLASS).css('width', width)
                    .on("click", sgMousedownHandler);
                borderRight = $('<div/>').addClass(BORDER_CLASS).css('width', width)
                    .on("click", sgMousedownHandler);
