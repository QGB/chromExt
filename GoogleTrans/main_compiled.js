...
(function() {
	...
	;
	var B = function(a) {
		if (a instanceof A && a.constructor === A && a.a === pa)
			return a.s;
	...
	var va = D("Opera"), E = D("Trident") || D("MSIE"), wa = D("Edge"), F = D("Gecko") && !(-1 != C.toLowerCase().indexOf("webkit") && !D("Edge")) && !(D("Trident") || D("MSIE")) && !D("Edge"), xa = -1 != C.toLowerCase().indexOf("webkit") && !D("Edge"), ya = function() {
		var a = k.document;
	...
	var Sa = function(a, b) {
		...
		if (-1 != String(k.Promise).indexOf("[native code]")) {
			...
			L = function() {
				var a = Ta;
				!p(k.setImmediate) || k.Window && k.Window.prototype && !D("Edge") && k.Window.prototype.setImmediate == k.setImmediate ? (Ka || (Ka = La()),
				Ka(a)) : k.setImmediate(a)
			}
	}, Qa = !1, Ra = new Ma, Ta = function() {
		...
		if (0 == a.a)
			if (a.c) {
				...
				if (c.b) {
					...
					!(e && 1 < d))); g = g.next)
						e || (f = g);
					e && (0 == c.a && 1 == d ? $a(c, b) : (f ? (d = f,
					y(c.b),
					y(null != d),
					d.next == c.f && (c.f = d),
					d.next = d.next.next) : ab(c),
					bb(c, e, 3, b)))
				}
				a.c = null
			} else
...
*/
	...
	P.prototype.cancel = function(a) {
		if (this.a)
			this.c instanceof P && this.c.cancel();
		else {
			if (this.b) {
				var b = this.b;
				delete this.b;
		...
		for (var d = c = !1; a.g.length && !a.i; ) {
			...
			if (f = a.f ? g : f)
				try {
					...
					void 0 !== h && (a.f = a.f && (h == b || a.isError(h)),
					a.c = b = h);
					if (Va(b) || "function" === typeof k.Promise && b instanceof k.Promise)
						d = !0,
						a.i = !0
				} catch (l) {
					b = l,
	...
	var tb = function(a, b) {
		...
		e.onload = e.onreadystatechange = function() {
			e.readyState && "loaded" != e.readyState && "complete" != e.readyState || (T(e, c.G || !1, h),
			lb(g),
			mb(null),
			jb(g, !0, null))
		}
		...
		e.onerror = function() {
			T(e, !0, h);
			ib(g, new rb(0,"Error while loading script " + d))
		}
		;
		f = c.attributes || {};
		oa(f, {
	...
	ub.prototype.send = function(a, b, c, d) {
		...
		var f = /\?/.test(e) ? "&" : "?";
		for (l in a)
			for (var g = "array" == n(a[l]) ? a[l] : [a[l]], h = 0; h < g.length; h++)
				null != g[h] && (e += f + encodeURIComponent(l) + "=" + encodeURIComponent(String(g[h])),
				f = "&");
		var l = qa(e);
		l = tb(l, b);
	...
	U.prototype.m = function(a) {
		...
		if (this.h) {
			...
			(864E5 < Math.abs(b - c) || d != a) && (new ub("https://translate.googleapis.com/translate_a/l","cb")).send({
				client: "gtx",
				hl: d
			}, q(this.l, this, d))
		}
	...
	Gb.prototype.popup = function() {
		this.a || this.v || (this.a = !0,
		this.b = "",
		chrome.tabs.detectLanguage(this.c, q(this.f, this)))
	}
	;
	Gb.prototype.f = function(a) {
		if (!this.v) {
			if (!a || "und" == a || a.match("invalid"))
				a = "auto";
			this.b = a;
			a = u(Bb());
			var b = u(this.b);
	...
	chrome.runtime.onMessage.addListener(function(a, b) {
		a.audioSrc ? W.paused || a.audioSrc != W.src ? (W.src = a.audioSrc,
		W.load(),
		W.play()) : (W.pause(),
		W.currentTime = 0) : a.bubbleClosed || a.popupClosed ? W.pause() : a.detectLanguage && chrome.tabs.detectLanguage(function(a) {
	...
	for (var Z; X.length && (Z = X.shift()); )
		X.length || void 0 === Ib ? Y[Z] && Y[Z] !== Object.prototype[Z] ? Y = Y[Z] : Y = Y[Z] = {} : Y[Z] = Ib;
